...
export const ContentCard = ({ content }: Props) => {
  ...
  return (
    <Link to={`/articles/${content.id}`} className='max-w-[330px]'>
      <div className='border col-span-1 space-y-4 pb-4'>
        ...
        <div className='px-3 space-x-2'>
          {content.tags.map((tag) => (
            <Badge
              key={tag.id}
              variant='outline'
              className='h-8 space-x-1 font-medium'
            >
              <span>#</span>
              <span>{tag.name}</span>
            </Badge>
        ...
        <div className='px-4'>
          <p className='h-16 font-medium'>{content.title}</p>
